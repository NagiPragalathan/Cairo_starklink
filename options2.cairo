...
fn display_grades(student: @Student, index: usize) {
    ...
    if course.is_some() {
        // If there is a grade, unwrap and print it
        println!("grade is {}", course.unwrap());
    } else {
        // If there is no grade, print "No grade"
        println!("No grade");
    }
...
fn test_all_defined() {
    ...
    let mut student = Student { name: 'Alice'.into(), courses: courses };
...
fn test_some_empty() {
    ...
    let mut student = Student { name: 'Bob'.into(), courses: courses };
    display_grades(@student, 0);
}